...
const (
	...
	LeafTypeMessage = uint8(1)
        GetClaimTxsByStatusLimit = 100    //zkbtc
)
...
type ClaimTxManager struct {
	...
	synced          bool
	monitorTxsSyncing atomic.Value  //zktbc
...
func (tm *ClaimTxManager) Start() {
	...
	tm.monitorTxsSyncing.Store(false) //zkbtc
	for {
		select {
		case <-tm.ctx.Done():
			return
...
func (tm *ClaimTxManager) ReviewMonitoredTx(ctx context.Context, mTx *ctmtypes.MonitoredTx, reviewNonce bool) error {
	...
	if reviewNonce {
		// check nonce
		nonce, err := tm.getNextNonce(mTx.From, true)//zkbtc
		if err != nil {
